...
export function querySelectorAll(context: HTMLElement | Document | null, selector: string) {
  if (!context) {
    return []
  }
...
export function querySelectors(context: HTMLElement | Document, selectors: string[]) {
  return selectors.map((selector) => context.querySelector<HTMLElement>(selector))
}
